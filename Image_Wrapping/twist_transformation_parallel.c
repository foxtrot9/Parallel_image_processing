...
void bilinearlyInterpolateParallel(PPMPixel *inPixel,PPMPixel *outPixel, int columns, int rows, double x, double y,int outX,int outY)
{
...
PPMImage * twist_transformation_parallel(PPMImage *inImage,int noOfThreads)
{
    double PI = 3.141592;
    ...
    // Module 2: 2 lines
    /*
    PPMPixel *inPointer;  // Index of a element in input PPMPixel array.
    PPMPixel *outPointer; // Index of a element in output PPMPixel array.
     */
...
#   pragma omp parallel for private (loopy,newX,newY,theta,radius) num_threads(noOfThreads)
    for(loopx=0;loopx < rows;++loopx)
    {
        for (loopy=0; loopy < columns; ++loopy)
        {
            ...
            bilinearlyInterpolateParallel(inPixel,outPixel,columns,rows, newX, newY,loopx,loopy);
        }
